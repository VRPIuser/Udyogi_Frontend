import { ABCTechnologies, DataCrunchersInc, VRPIGroup } from "./CompaniesData";

export const LatestJobsData = [
  {
    jobId: "1",
    jobTitle: "Software Engineer",
    location: "Hyderabad",
    workType: "Remote",
    jobType: "Full time",
    shift: "Day",
    postedDate: "2022-01-15T08:30:00",
    qualification: "Bachelor Degree",
    shortDescriptionData:
      "WNon fringilla nisl rutrum magna et risus dui nulla. Fringilla nisl rutrum magna et risus dui nulla.",
    expectedSkills: ["JavaScript", "React", "Node.js"],
    careerLevel: "Senior",
    salaryRange: {
      upperLimit: 6,
      lowerLimit: 3,
      currency: "INR",
      salaryType: "LPA",
    },
    applicants: ["1", "2", "3", "4", "5", "6"],
    expectedExperience: {
      upperLimit: 6,
      lowerLimit: 3,
      experienceMetrics: "years",
    },
    jobDescription: {
      overview:
        "We are Uxper. With a presence in more than 60 countries, we’re a growing global organization that helps amazing companies engage with customers through messaging, email, voice and video.",
      requirements:
        "Be heavily involved in turning user stories into testable, maintainable and high-quality code. This is a hands-on code design and coding role!\n\nBe a valued member of an autonomous, cross-functional team delivering our messaging experience to businesses around the world.\n\nPromote and share knowledge for improvement of methodologies and best practices.\n\nClose-knitted collaboration with equally passionate team members having fun at work and feeling proud that you are a key part of creating world-class solutions for customer engagement.",
    },

    companyDetails: VRPIGroup,
  },
  {
    jobId: "2",

    jobTitle: "Data Analyst",
    location: "Bangalore",
    workType: "On Site",
    jobType: "Part time",
    shift: "Day",
    careerLevel: "Junior",
    postedDate: "2022-01-15T08:30:00",
    qualification: "Bachelor Degree",

    shortDescriptionData:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer ullamcorper nisl ut nunc ullamcorper, ac consectetur magna luctus.",
    expectedSkills: ["SQL", "Python", "Data Visualization"],
    // salaryRange: {
    //   upperLimit: 90000,
    //   lowerLimit: 60000,
    //   currency: "USD",
    //   salaryType: "Per Annum",
    // },
    salaryRange: {
      upperLimit: 4,
      lowerLimit: 2,
      currency: "INR",
      salaryType: "LPA",
    },

    applicants: ["1", "2", "3", "4", "5", "6"],
    expectedExperience: {
      upperLimit: 0,
      lowerLimit: 0,
      experienceMetrics: "years",
    },

    jobDescription: {
      overview:
        "We are Uxper. With a presence in more than 60 countries, we’re a growing global organization that helps amazing companies engage with customers through messaging, email, voice and video.",
      requirements:
        "Be heavily involved in turning user stories into testable, maintainable and high-quality code. This is a hands-on code design and coding role!\n\nBe a valued member of an autonomous, cross-functional team delivering our messaging experience to businesses around the world.\n\nPromote and share knowledge for improvement of methodologies and best practices.\n\nClose-knitted collaboration with equally passionate team members having fun at work and feeling proud that you are a key part of creating world-class solutions for customer engagement.",
    },
    companyDetails: VRPIGroup,
  },
  {
    jobId: "3",

    jobTitle: "UX Designer",
    location: "Bangalore",
    workType: "Hybrid",
    jobType: "Contract",
    shift: "Night",
    careerLevel: "Junior",
    qualification: "Bachelor Degree",

    postedDate: "2022-01-15T08:30:00",
    shortDescriptionData:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer ullamcorper nisl ut nunc ullamcorper, ac consectetur magna luctus.",
    expectedSkills: ["UI/UX Design", "Wireframing", "Prototyping"],
    // salaryRange: {
    //   upperLimit: 120000,
    //   lowerLimit: 80000,
    //   currency: "USD",
    //   salaryType: "Per Year",
    // },
    salaryRange: {
      upperLimit: 8,
      lowerLimit: 6,
      currency: "INR",
      salaryType: "LPA",
    },

    applicants: ["1", "2", "3", "4", "5", "6"],
    expectedExperience: {
      upperLimit: 8,
      lowerLimit: 3,
      experienceMetrics: "years",
    },
    jobDescription: {
      overview:
        "We are Uxper. With a presence in more than 60 countries, we’re a growing global organization that helps amazing companies engage with customers through messaging, email, voice and video.",
      requirements:
        "Be heavily involved in turning user stories into testable, maintainable and high-quality code. This is a hands-on code design and coding role!\n\nBe a valued member of an autonomous, cross-functional team delivering our messaging experience to businesses around the world.\n\nPromote and share knowledge for improvement of methodologies and best practices.\n\nClose-knitted collaboration with equally passionate team members having fun at work and feeling proud that you are a key part of creating world-class solutions for customer engagement.",
    },
    companyDetails: ABCTechnologies,
  },
  {
    jobId: "4",
    careerLevel: "Senior",

    jobTitle: "Frontend Developer",
    location: "Bangalore",
    workType: "Remote",
    jobType: "freelancer",
    shift: "Rotational",
    qualification: "Bachelor Degree",

    postedDate: "2022-01-15T08:30:00",
    shortDescriptionData:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer ullamcorper nisl ut nunc ullamcorper, ac consectetur magna luctus.",
    expectedSkills: ["HTML", "CSS", "JavaScript", "React"],
    // salaryRange: {
    //   upperLimit: 100000,
    //   lowerLimit: 70000,
    //   currency: "USD",
    //   salaryType: "Per Annum",
    // },
    salaryRange: {
      upperLimit: 10,
      lowerLimit: 8,
      currency: "INR",
      salaryType: "LPA",
    },

    applicants: ["1", "2", "3", "4", "5", "6"],
    expectedExperience: {
      upperLimit: 10,
      lowerLimit: 5,
      experienceMetrics: "years",
    },
    jobDescription: {
      overview:
        "We are Uxper. With a presence in more than 60 countries, we’re a growing global organization that helps amazing companies engage with customers through messaging, email, voice and video.",
      requirements:
        "Be heavily involved in turning user stories into testable, maintainable and high-quality code. This is a hands-on code design and coding role!\n\nBe a valued member of an autonomous, cross-functional team delivering our messaging experience to businesses around the world.\n\nPromote and share knowledge for improvement of methodologies and best practices.\n\nClose-knitted collaboration with equally passionate team members having fun at work and feeling proud that you are a key part of creating world-class solutions for customer engagement.",
    },
    companyDetails: ABCTechnologies,
  },
  {
    jobId: "5",
    careerLevel: "Junior",

    jobTitle: "Backend Developer",
    location: "Chennai",
    workType: "On Site",
    jobType: "Contract",
    shift: "Day",
    qualification: "Bachelor Degree",

    postedDate: "2022-01-15T08:30:00",
    shortDescriptionData:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer ullamcorper nisl ut nunc ullamcorper, ac consectetur magna luctus.",
    expectedSkills: ["Java", "Spring", "SQL"],
    // salaryRange: {
    //   upperLimit: 120000,
    //   lowerLimit: 80000,
    //   currency: "USD",
    //   salaryType: "Per Annum",
    // },
    salaryRange: {
      upperLimit: 3,
      lowerLimit: 2,
      currency: "INR",
      salaryType: "LPA",
    },

    applicants: ["1", "2", "3", "4", "5", "6"],
    expectedExperience: {
      upperLimit: 15,
      lowerLimit: 10,
      experienceMetrics: "years",
    },
    jobDescription: {
      overview:
        "We are Uxper. With a presence in more than 60 countries, we’re a growing global organization that helps amazing companies engage with customers through messaging, email, voice and video.",
      requirements:
        "Be heavily involved in turning user stories into testable, maintainable and high-quality code. This is a hands-on code design and coding role!\n\nBe a valued member of an autonomous, cross-functional team delivering our messaging experience to businesses around the world.\n\nPromote and share knowledge for improvement of methodologies and best practices.\n\nClose-knitted collaboration with equally passionate team members having fun at work and feeling proud that you are a key part of creating world-class solutions for customer engagement.",
    },
    companyDetails: DataCrunchersInc,
  },
  {
    jobId: "6",
    jobTitle: "Product Manager",
    location: "Gurgaon",
    workType: "Hybrid",
    jobType: "Internship",
    shift: "Rotational",
    careerLevel: "Senior",
    qualification: "Bachelor Degree",

    postedDate: "2022-01-15T08:30:00",
    shortDescriptionData:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer ullamcorper nisl ut nunc ullamcorper, ac consectetur magna luctus.",
    expectedSkills: [
      "Product Management",
      "Market Research",
      "Agile Methodologies",
    ],
    salaryRange: {
      upperLimit: 4,
      lowerLimit: 4,
      currency: "INR",
      salaryType: "LPA",
    },
    applicants: ["1", "2", "3", "4", "5", "6"],
    expectedExperience: {
      upperLimit: 2,
      lowerLimit: 1,
      experienceMetrics: "years",
    },
    jobDescription: {
      overview:
        "We are Uxper. With a presence in more than 60 countries, we’re a growing global organization that helps amazing companies engage with customers through messaging, email, voice and video.",
      requirements:
        "Be heavily involved in turning user stories into testable, maintainable and high-quality code. This is a hands-on code design and coding role!\n\nBe a valued member of an autonomous, cross-functional team delivering our messaging experience to businesses around the world.\n\nPromote and share knowledge for improvement of methodologies and best practices.\n\nClose-knitted collaboration with equally passionate team members having fun at work and feeling proud that you are a key part of creating world-class solutions for customer engagement.",
    },
    companyDetails: DataCrunchersInc,
  },
];
