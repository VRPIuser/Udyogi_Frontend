const UsersTable = ({ tableData }) => {
  ...
  return (
    <div className="h-fit" style={{ minHeight: "500px" }}>
      <table className={`${tableClasses}`}>
        <thead>
          <tr className={headerRowClasses}>
            ...
            <th className={`${thClasses}  text-center`}>No.Of Posts</th>
            <th className={`${thClasses} text-center`}>Applications</th>
            <th className={`${thClasses} text-start`}>Date of Post</th>
        ...
        <tbody className={fontClasses}>
          {tableData?.map((rowData, index) => (
            <tr key={index} className={rowClasses}>
              ...
              <td className={tdClasses}>{rowData.dateOfPost}</td>
              <td className={`${tdClasses} text-center`}>
                <span
